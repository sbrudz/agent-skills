...
| [boy-scout-rule](skills/boy-scout-rule/SKILL.md) | Concrete refactoring checklist for after tests pass, before committing, and during code review. Covers structural clarity (extract function, reduce nesting, simplify conditionals), structural design (consolidate duplication, right-size functions), and cross-cutting patterns. References functional-core-imperative-shell and react-best-practices as companion checks. |
| [visual-feedback-loop](skills/visual-feedback-loop/SKILL.md) | Visual feedback loop after implementing UI changes or before committing. Screenshots the running application to verify UI correctness (layout, spacing, text, completeness) before refactoring. Tool-agnostic — works with any browser automation (Puppeteer, Chrome DevTools MCP, etc.). Scoped to UI-affecting tasks only. |
| [ux-design-principles](skills/ux-design-principles/SKILL.md) | User-centered design thinking before writing code for any user-facing feature. Applies Goal-Directed Design (Cooper) and usability principles (Krug) — user goals, context, expectations, progressive disclosure, scanning priority. |
...
| [writing-clear-tech-docs](skills/writing-clear-tech-docs/SKILL.md) | Four-pass layered writing process for creating and revising technical documents (design docs, RFCs, ADRs) intended for fellow engineers. Encodes principles from Minto, Williams, Zinsser, and others into concrete revision passes: structure (pyramid, SCQA), hierarchy (C4 zoom levels), clarity (given-new, concision, concrete anchoring), and reader calibration (curse of knowledge countermeasures, pointer audit for ideas referenced by label rather than stated). Core principle: revise by subtraction and restructuring, never by addition. |
